...
{
    public partial class ThayDoiTacGia : Form
    {
        public ThayDoiTacGia()
        {
            InitializeComponent();
        }
        ...
        private void ThayDoiTacGia_Load(object sender, EventArgs e)
        {
        ...
        {
            if (txtNhapTacGiaMoi.Text != "Nhập tên tác giả mới")
            {
                using (DataBaseForLibraryDataContext db = new DataBaseForLibraryDataContext())
                {
                    Sach sach = db.Saches.FirstOrDefault(s => s.MaSach == txtMaSach_TenSach.Text.Trim() || s.TenSach == txtMaSach_TenSach.Text.Trim());
                    sach.TacGia = txtNhapTacGiaMoi.Text;
                    db.SubmitChanges();
                    db.Refresh(System.Data.Linq.RefreshMode.KeepCurrentValues, sach);
                    MessageBox.Show("Thay đổi thành công!\nTác giả hiện tại là: " + sach.TacGia, "Thông báo");
        ...
        private void txtNhapTacGiaMoi_Enter(object sender, EventArgs e)
        {
            if (txtNhapTacGiaMoi.Text == "Nhập tên tác giả mới")
            {
                txtNhapTacGiaMoi.Clear();
                txtNhapTacGiaMoi.ForeColor = Color.Black;
            }
        }

        private void txtNhapTacGiaMoi_Leave(object sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(txtNhapTacGiaMoi.Text))
            {
                txtNhapTacGiaMoi.Text = "Nhập tên tác giả mới";
                txtNhapTacGiaMoi.ForeColor = Color.Gray;
            }
        }
    }
}
